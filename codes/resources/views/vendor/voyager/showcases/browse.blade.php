                        ...
                        <div class="dashboard">
                            ...
                            @php
                                if (
                                    ...
                                    $showcases = App\Showcase::where('deliveryboy_id', auth()->user()->id)
                                    ...
                                    $showcases = App\Showcase::where('pickup_city', auth()->user()->city)
                                        ->where('is_order_accepted', true)
                                        ->get();
                                } elseif (
                                    auth()
                            ...
                            @if (auth()->user()->hasRole(['Vendor', 'Client', 'admin']))
                                <a href="/{{ Config::get('icrm.admin_panel.prefix') }}/showcases?label=New Order">
                                    <div
                                        class="item @if (request('label') == 'New Order') new_order active @endif
                                ...
                                <a href="/{{ Config::get('icrm.admin_panel.prefix') }}/showcases?label=Accepted">
                                    <div class="item @if (request('label') == 'Accepted') active @endif">
                                        @if (auth()->user()->hasRole(['Vendor']))
                                            <div class="stat">
                                                <span
                                                    class="count">{{ $showcases->where('vendor_id', auth()->user()->id)->whereIn('order_status', ['Accepted'])->count() }}</span>
                                            </div>
                                        @elseif(auth()->user()->hasRole(['Client', 'admin']))
                                            <div class="stat">
                                                <span
                                                    class="count">{{ $showcases->whereIn('order_status', ['Accepted'])->count() }}</span>
                                            </div>
                                        @endif
                                ...
                                <a href="/{{ Config::get('icrm.admin_panel.prefix') }}/showcases?label=Non Acceptance">
                                    <div class="item @if (request('label') == 'Non Acceptance') cancelled active @endif">
                                        @if (auth()->user()->hasRole(['Vendor']))
                                            <div class="stat">
                                                <span
                                                    class="count">{{ $showcases->where('vendor_id', auth()->user()->id)->where('is_order_accepted', false)->whereIn('order_status', ['Non Acceptance'])->count() }}</span>
                                            </div>
                                        @elseif(auth()->user()->hasRole(['Client', 'admin']))
                                            <div class="stat">
                                                <span
                                                    class="count">{{ $showcases->where('is_order_accepted', false)->whereIn('order_status', ['Non Acceptance'])->count() }}</span>
                                            </div>
                                        @endif
                                    ...
                                    @foreach ($dataTypeContent as $data)
                                        <tr
                                            ...
                                            <td class="no-sort no-click bread-actions">
                                                ...
                                                @if (auth()->user()->hasRole(['Vendor']) &&
                                                        !$data->is_order_accepted &&
                                                        $data->order_status === 'New Order')
                                                    <div class="mx-auto">
                                                        <p style="color:black; font-size: 14px; font-weight:500;">New
                                                            Showroom Order</p>
                                            ...
                                            <td>
                                                <div>
                                                    @if (!$data->is_order_accepted && $data->order_status == 'Non Acceptance')
                                                        <span
                                                            style="color: black; font-width: 400;">{{ $data->order_status }}</span>
                                                        <span style="color: red;font-weight:600; font-size:15px"
                                                            title="Not Acceptance charges">
                                                            <br>NAC:
                                                            {{ Config::get('icrm.currency.icon') . ' ' . round($data->product_offerprice * 0.1035, 0) }}
                                                            /-
                                                        </span>
