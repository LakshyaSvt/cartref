                                            ...
                                            <td class="product-thumbnail">
                                                <figure>
                                                    <a
                                                        ...
                                                        @endphp
                                                        <img src="{{ Voyager::image($colorimage) }}" width="100"
                                                             height="100"
                                                ...
                                                <div class="product-name-section">
                                                    ...
                                                    @isset($product->manufacturing_period)
                                                        @if ($product->manufacturing_period > 0)
                                                            <br><span>Manufacturing Period:
                                                                    {{ $product->manufacturing_period }} days</span>
                                                        @endif
                                                    @endif
                                                ...
                                                <td class="product-quantity">
                                                    <div class="input-group">
                                                        ...
                                                        <button class="quantity-plus d-icon-plus"
                                                                wire:click="plusqty({{ $cart->id }}, {{ $cart->attributes->weight }})"></button>
                                                    </div>
                            ...
                            <span class="amount">{{ Config::get('icrm.currency.icon') }}
                                ...
                                <del>{{ Config::get('icrm.currency.icon') }}{{ $product->mrp * $cart->quantity }}</del>
                                            ...
                                            <td class="product-close">
                                                <a wire:click="removecart({{ $cart->id }})" class="product-remove"
                                                   title="Remove this product">
                                                    <i class="fas fa-times"></i>
                                                </a>
                                            </td>
                                            <td class="product-close">
                                                <a wire:click="wishlist({{ $product->id }},{{ $cart->id }})" class="product-remove"
                        ...
                        @else
                            <div class="p-20 mb-4 bg-light rounded-3 text-center">
                                <div class="container-fluid py-5">
                                    <img src="{{ asset('images/icrm/wishlist/empty_wishlist.svg') }}" class="img-responsive"
                                         alt="wishlist empty">
                                    <h1 class="display-5 fw-bold text-dark">Your {{ Config::get('icrm.cart.name') }} is empty
                                    </h1>
                                    <p class="fs-4 text-center">Browse products and add your favourite ones in
                                        {{ Config::get('icrm.cart.name') }}</p>
                                    <a href="{{ route('products') }}" class="btn btn-primary btn-lg" type="button">Browse
                                        Products</a>
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="summary mb-4">
                                ...
                                <table class="shipping">
                                    <tr class="summary-subtotal">
                                        ...
                                        <td>
                                            <p class="summary-subtotal-price">{{ Config::get('icrm.currency.icon') }}
                                ...
                                <div class="shipping-address">
                                    <form wire:submit.prevent="checkshippingavailability">
                                        <input type="text"
                                               ...
                                               wire:model.defer="deliverypincode" placeholder="Delivery Pincode"/>
                                        ...
                                        @elseif(Session::has('deliverynotavailable') == true)
                                            <small class="notavailable">{{ Session::get('deliverynotavailable') }}</small>
                                            <br>
                                        @endif

                                        <button type="submit"
                                                class="btn btn-md btn-dark btn-rounded btn-outline">Check
                                        </button>
                                    </form>
                                </div>
                                <table class="total">
                                    <tr class="summary-subtotal">
                                        ...
                                        <td>
                                            <p class="summary-total-price ls-s">{{ Config::get('icrm.currency.icon') }}
