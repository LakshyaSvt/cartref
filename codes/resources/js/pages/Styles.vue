  ...
  <div>
    <div class="container mx-auto my-2 px-4">
      <div class="flex gap-2 items-center text-3xl text-primary-600 font-semibold">
        ...
        <h3 class="text-start my-8">Styles</h3>
      </div>

      <div class="bg-white p-4 overflow-x-auto shadow-md sm:rounded-lg my-4">
        <div class="block">
          <form @submit.prevent="editOrCreateStyle()">
            <div class="md:flex mb-3">
              <div class="mb-5 md:w-1/2 w-full mx-2 my-1">
                <label for="style" class="block mb-2 text-sm font-bold text-gray-900" title="The name is how it appears on your site.">Style <span
                    class="text-red-600">*</span></label>
                <input type="text" v-model="name" id="style"
                       class="form-input"
        ...
        <div class="block">
          <div class="flex flex-wrap items-center justify-between py-4">
            <div class="flex flex-wrap text-base text-gray-700 gap-2">
              ...
              <div>
                <button :disabled="!pagination.prev_page_url" @click="fetchStyle(pagination.prev_page_url)" title="Previous"
                        class="border border-transparent rounded-full hover:bg-primary-400 disabled:opacity-50">
                  <i class="fi fi-rr-angle-small-left text-xl px-1 py-2"></i>
                </button>
                <button :disabled="!pagination.next_page_url" @click="fetchStyle(pagination.next_page_url)" title="Next"
                        class="border border-transparent rounded-full hover:bg-primary-400 disabled:opacity-50">
            ...
            <div class="flex flex-wrap items-center gap-2">
              <div class="relative">
                <select title="Status" v-model="status" @change="fetchStyle()" class="filter-dropdown">
              ...
              <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 cursor-pointer"
                     @click="keyword = ''; fetchStyle();" v-if="keyword">
                ...
                <input type="text" v-model="keyword" class="search" placeholder="Search" @keydown.enter="fetchStyle()">
              </div>
              <div class="flex border border-gray-600 rounded-lg bg-white">
                <button class="px-2 py-1 m-[2px] hover:bg-primary-100 border-r border-solid cursor-pointer" @click="fetchStyle()">
                  <i class="ffi fi-rr-refresh mr-1"></i>
                </button>
                <select class="w-14 block px-1 m-[2px] text-base text-center text-gray-900 bg-white cursor-pointer" @change="fetchStyle()" v-model="row_count">
          ...
          <template v-else-if="styles && styles.length > 0">
            <div class="clear-right overflow-x-auto">
              <div class="table border-solid border border-gray-500 w-full">
                ...
                <div v-for="(s, index) in styles" v-bind:key="index" class="table-row table-body hover:bg-primary-100" :class="{'bg-primary-200': s.id === editId}">
                  ...
                  <div class="table-cell border-t border-l border-gray-500 text-sm align-[middle!important] text-center">
                    <div class="flex gap-4 items-center justify-center">
                      <a href="javascript:void(0)" @click="editStyle(s.id)" type="button"
                         class="font-medium cursor-pointer text-yellow-500">
                        <i class="fi fi-rr-pencil w-5 h-5 text-xl"></i>
                      </a>
                      <a href="javascript:void(0)" @click="deleteStyle(s.id)" type="button"
              ...
              <div class="flex items-center justify-between py-4">
                ...
                <Pagination :pagination="pagination" :fetchNewData="fetchStyle"/>
              </div>
            </div>
          </template>
          <template v-else>
...
export default {
  ...
  methods: {
    ...
    updateStatus(id, status) {
      axios.put('/admin/style/' + id, {status})
          .then(res => {
            this.show_toast(res.data.status, res.data.msg);
            let index = this.styles.findIndex(style => style.id === id)
            this.$set(this.styles, index, res.data.data)
          })
          .catch(err => {
